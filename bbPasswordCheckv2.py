...
# Forcing script to wait for a desktop environment before executing the rest
...
USERNAME = "bbuser" # Default BeaglePlay user = "bbuser"
MIN_LENGTH = 12
MIN_DICT_WORD_LEN = 4
...
# Checking password against all parameters and returning reasons for rejection
...
# This sets the password for the bbuser account to the user's password
# given it passes all checks

def set_system_password(username: str, password: str):
    try:
        proc = subprocess.run(
            ["passwd", username],
            input=(password + "\n" + password + "\n").encode(),
...
def main():
    print("Settting password for bbuser. Ensure that you will remember this password.")
    ...
    while True:
        ...
        ok = set_system_password(USERNAME,pw1)
        if ok:
